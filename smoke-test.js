...
(function () {
  ...
  check('无仓库时为基础储量', (function () {
    ...
    return G.storeCap() === 2000000;
  })(), U.fmt(G.storeCap()));
  check('建造仓库', putBldg('cangku', 3));
  check('仓库提升储量上限（每级+200万）', G.storeCap() === 6000000, U.fmt(G.storeCap()));
  ...
  (function () {
    ...
    check('成品进入装备背包', (S23.inventory || []).length === inv0 + 1 && S23.inventory.indexOf('cr_head_1') >= 0);
    ...
    check('倚天长剑入背包', S23.inventory.indexOf('yt_sword') >= 0);
  ...
  check('已穿装备拆解前会先卸下', (function () {
    var gg = G.state.generals[0];
    var m = G.makeGeneral('拆解测试', 3, 'idle', G.state.cities[0].id, false);
    G.state.generals.push(m);
    G.state.inventory.push('cr_chest_2');
    G.systems.equipItem(m.id, 'cr_chest_2');
    G.state.inventory.push('cr_chest_2');
    var r = G.salvageEquip('cr_chest_2');
    var wornGone = !(m.equip || {}).chest || m.equip.chest !== 'cr_chest_2';
    return r.ok && wornGone;
  })());
...
(function () {
  ...
  check('结构：仓储上限按城（storeCap → storeCapOf）', (function () {
    return /GAME\.storeCapOf = function/.test(dS)
      && /storeCapOf\(GAME\.currentCity\(\)\)/.test(codeOf(dS, 'GAME.storeCap = function'))
      && /perkNum\(city, 'storePct'\)/.test(codeOf(dS, 'GAME.storeCapOf = function'));
  })());
  ...
  check('结构：perk 每一项都真有消费点（死属性检查法）', (function () {
    return /perkNum\(city, 'prodPct'\)/.test(codeOf(stS, 'GAME.cityProdPerSec = function'))
      && /perkNum\(city, 'taxPct'\)/.test(codeOf(stS, 'GAME.cityProdPerSec = function'))
      && /perkNum\(city, 'storePct'\)/.test(codeOf(dS, 'GAME.storeCapOf = function'))
      && /perkNum\(city, 'buildSlot'\)/.test(codeOf(dS, 'GAME.buildSlots = function'))
