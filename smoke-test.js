...
(function () {
  ...
  check('实测：携妖言开战守军 = 原守军 ×0.85 + 战报注明', (function () {
    ...
    gen.energy = 100;
    GAME.setStaNow(gen, 100);
    /* v88 维护：原选「第一块野地」—— 小守军时因逐兵种 Math.round(0.85x) 的取整误差
       会偶发偏离断言（0.85 精度不足）。改为找一块**守军 >=300** 的野地：大样本下
       逐兵种取整误差 <1%。resolveTarget 对野地是确定性的（wildDefenseAt 生成），扫描安全。 */
    var wt3 = null;
    for (var yy = 5; yy < 80 && !wt3; yy++) {
      for (var xx = 5; xx < 80; xx++) {
        var tl = GAME.map.tile(xx, yy);
        if (!tl || tl.terrain === 'city' || GAME.map.fortAt(xx, yy)) continue;
        var cand3 = { kind: 'wild', x: xx, y: yy };
        var tt3 = GAME.battle.resolveTarget(cand3);
        var gs3 = 0;
        for (var gk3 in (tt3.garrison || {})) gs3 += tt3.garrison[gk3];
        if (gs3 >= 300) { wt3 = cand3; break; }
      }
    }
    if (!wt3) return false;
    ...
    /* v88：容差 0.02 -> 0.03（大守军下逐兵种取整误差 <1%，再留余量） */
    return Math.abs(dA / gSum - 0.85) < 0.03;
  })());
