...
(function () {
   ...
   * 新版：**按城**征收，入口放进「官府」建筑内 —— 征收本就是官府的事：
   *   · 普通城池（自建城）→ 征 5 大资源（粮 / 木 / 石 / 铁 / 金）
   *   · 名城（攻占的都/州/郡/县城）→ 征该州**特产高阶材料**
   * 代价：民力不可白取，一律扣民心；冷却 1 游戏小时，**按城独立**。
   * ============================================================ */
  GAME.LEVY_CD = 3600;      // 游戏秒

  /* 5 大资源的征收系数，基数为「本城人口上限 × 民心%」。
  ...
  DATA.LEVY_HEARTS = { self: 2, other: 4 };

  /* 距可征收的剩余**游戏秒**（0 = 现在可征）。按城独立 ——
     多城经营时不该"A 城征完，B 城也不能征"。 */
  GAME.levyReady = function (city) {
    var s = GAME.state;
    if (!s) return 0;
    city = city || GAME.currentCity();
    if (!city) return GAME.LEVY_CD;
    var now = (s.world && s.world.elapsed) || 0;
    /* lastLevy 为 null 表示从未征收过（游戏起始 elapsed 也是 0，不能用 0 当哨兵） */
    if (city.lastLevy == null) return 0;
    return Math.max(0, GAME.LEVY_CD - (now - city.lastLevy));
  };

  /* 本城这次能征到什么：{ self, resources:[{key,name,icon,qty}], hearts, cd, specialty } */
  GAME.levyPlan = function (city) {
    city = city || GAME.currentCity();
    if (!city) return null;
    var s = GAME.state;
    var hearts = Math.max(0, Math.min(100, s.hearts == null ? 100 : s.hearts));
    var base = GAME.maxPopOf(city) * hearts / 100;
    var isSelf = (city.type || 'self') === 'self';
    var rows = [];
    if (isSelf) {
      DATA.RES_ORDER.forEach(function (k) {
        ...
        rows.push({
          key: k, name: meta ? meta.name : k, icon: meta ? meta.icon : '',
          qty: Math.round(base * (DATA.LEVY_RES_RATE[k] || 0)),
        });
      });
    } else {
      var sp = GAME.specialtyOf(city);
      if (sp) {
        ...
        var seat = GAME.hasStateSeat(GAME.stateOfCity(city));
        var mul = seat ? (DATA.STATE_SEAT_BONUS || 1) : 1;
        rows.push({
          key: sp.mat, name: m ? m.name : sp.mat, icon: '🧵', tier: sp.tier, seat: seat,
          qty: Math.max(1, Math.round((DATA.LEVY_MAT_QTY[city.type] || 1) * mul)),
        });
      }
    }
    return {
      city: city, self: isSelf, resources: rows,
      hearts: isSelf ? DATA.LEVY_HEARTS.self : DATA.LEVY_HEARTS.other,
      cd: GAME.levyReady(city),
      specialty: isSelf ? null : GAME.specialtyOf(city),
      stateName: GAME.stateOfCity(city),
    };
  };

  GAME.levy = function (cityId) {
    var s = GAME.state;
    var city = cityId ? GAME.cityById(cityId) : GAME.currentCity();
    if (!city) return { ok: false, msg: '城池不存在' };
    var cd = GAME.levyReady(city);
    if (cd > 0) return { ok: false, msg: '征收冷却中（还需 ' + U.durExact(cd / GAME.timeScale()) + '）' };
    var plan = GAME.levyPlan(city);
    var rows = (plan && plan.resources) || [];
    if (!rows.length) return { ok: false, msg: '此城无可征收之物' };

    /* 先算出实际到手量（仓库满则按剩余容量打折），再决定是否扣民心 ——
       否则会出现"仓库全满、民心白扣"。黄金不受仓库上限约束（同产出/岁贡口径）。 */
    var cap = GAME.storeCap ? GAME.storeCap() : 0;
    var got = [];
    rows.forEach(function (r) {
      if (plan.self && r.key !== 'gold' && cap > 0) {
        r.got = Math.round(Math.max(0, Math.min(r.qty, cap - (s.res[r.key] || 0))));
      } else {
        r.got = r.qty;
      }
      if (r.got > 0) got.push(r);
    });
    if (!got.length) {
      return { ok: false, msg: plan.self ? '仓库已满，征无可征（先扩建仓库或消耗资源）' : '无可征收之物' };
    }
    s.items = s.items || {};
    got.forEach(function (r) {
      if (plan.self) s.res[r.key] = (s.res[r.key] || 0) + r.got;
      else s.items[r.key] = (s.items[r.key] || 0) + r.got;
    });
    s.hearts = Math.max(0, (s.hearts || 0) - plan.hearts);
    city.lastLevy = (s.world && s.world.elapsed) || 0;
    var txt = got.map(function (r) { return r.name + ' +' + (plan.self ? U.fmt(r.got) : r.got); }).join('、');
    GAME.log('💰 ' + city.name + ' 征收：' + txt + '（民心 -' + plan.hearts + '）');
    return { ok: true, msg: '征收：' + txt + '（民心 -' + plan.hearts + '）' };
  };
